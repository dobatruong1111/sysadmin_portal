...
import { useDeleteExtensionTypeMutation, useGetOneExtensionTypeQuery } from "../../api/apiExtensionType"
...
export const ConnectedExtensionTypeEditModal = () => {
    ...
    const selectedRow = useSelector((state: any) => state.tableReducer.data[TABLE_EXTENSION_TYPE].selection.selectedRow);
        ...
        if (selectedRow) {
            notifyModal({
                ...
                options: {
                    variant: 'warning',
                    onConfirm: async () => {
                        const result = await deleteExtensionType({ id: `${selectedRow.id}` });
                        if ('error' in result) {
                            notifySnackbar({
                                message: 'Lỗi',
                                options: {
...
export const ExtensionTypeEditModal = forwardRef<HTMLElement, ExtensionTypeEditModalProps>((props, ref) => {
