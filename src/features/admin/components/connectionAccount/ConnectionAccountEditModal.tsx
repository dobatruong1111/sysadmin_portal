...
export const ConnectedConnectionAccountEditModal = () => {
    ...
    const selectedRow = useSelector((state: any) => state.tableReducer.data[TABLE_CONNECTION_ACCOUNT].selection.selectedRow);
        ...
        if (selectedRow) {
            notifyModal({
                ...
                options: {
                    variant: 'warning',
                    onConfirm: async () => {
                        const result = await deleteConnectionAccount({ id: `${selectedRow.id}` });
                        if ('error' in result) {
                            notifySnackbar({
                                message: 'Lỗi',
                                options: {
...
export const ConnectionAccountEditModal = forwardRef<HTMLElement, ConnectionAccountEditModalProps>((props, ref) => {
